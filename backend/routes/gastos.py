...
from schemas import GastoResponse, GastoCriado, BalancoItem, MensagemResponse
...
class GastoInput(BaseModel):
    ...
    descricao: Optional[str] = Field(None, max_length=500)   # alinhado com DB VARCHAR(500)
...
class GastoUpdate(BaseModel):
    valor: float
    categoria: str = Field(..., max_length=50)
    descricao: str = Field(..., max_length=500)              # alinhado com DB VARCHAR(500)
...
@router.get("/grupos/{id_grupo}/gastos", response_model=list[GastoResponse])
def listar_gastos(
    id_grupo: int,
    ...
    return gasto_service.listar(id_grupo, usuario_id, limite, offset)


@router.post("/grupos/{id_grupo}/gastos", response_model=GastoCriado, status_code=201)
...
@router.get("/grupos/{id_grupo}/balanco", response_model=list[BalancoItem])
...
@router.put("/gastos/{id_gasto}", response_model=MensagemResponse)
...
@router.delete("/gastos/{id_gasto}", response_model=MensagemResponse)
