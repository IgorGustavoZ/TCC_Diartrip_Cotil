...
def listar(id_grupo: int, usuario_id: int, limite: int = 50, offset: int = 0) -> list:
    with get_db() as conexao:
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT f.id_foto, f.id_usuario, f.caminho_arquivo, f.template_usado, f.data_upload, u.nome
                FROM fotos f
...
def salvar(
    ...
    if not ext:
        ...
        logger.info("Extensão inferida via magic bytes: %s (nome original: %r)", ext, arquivo_nome)
