    ...
    with get_db() as conexao:
        ...
        try:
            ...
            cursor.execute(
                ...
                ORDER BY data_gasto DESC, id_gasto DESC LIMIT 5
                """,
                (id_grupo, usuario_id),
            )
            recentes = cursor.fetchall()
            for g in recentes:
                g["valor"] = round(float(g["valor"] or 0), 2)
                if g.get("data_gasto") is None:
                    g["data_gasto"] = date.today()
    ...
    with get_db() as conexao:
        ...
        try:
            ...
            cursor.execute(
                "SELECT valor, categoria, descricao, data_gasto "
                "FROM gastos "
                "WHERE id_grupo=%s AND id_usuario=%s ORDER BY data_gasto DESC, id_gasto DESC LIMIT 5",
                (id_grupo, usuario_id),
            )
            recentes = cursor.fetchall()
            for g in recentes:
            ...
            if cargo == "admin":
                ...
                for m in ranking:
                    m["total"] = round(float(m["total"] or 0), 2)
