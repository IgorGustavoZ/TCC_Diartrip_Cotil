...
CREATE TABLE IF NOT EXISTS grupos_viagem (
    ...
    -- Expiração do convite: NULL = convite permanente; valor = convite expira nesta data/hora
    codigo_validade   TIMESTAMP     NULL DEFAULT NULL,
    criado_por        INT           NOT NULL,
    -- data_criacao: timestamp de criação do grupo para auditoria e ordenação
    data_criacao      TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS gastos (
    ...
    -- VARCHAR(500) alinhado com o validador Pydantic max_length=500
...
CREATE TABLE IF NOT EXISTS chat_ia (
    ...
    PRIMARY KEY (id_chat),
    -- Índice composto cobre: WHERE id_usuario=%s AND id_grupo=%s ORDER BY data_interacao
    KEY idx_chatia_usuario_grupo_data (id_usuario, id_grupo, data_interacao),
...
CREATE TABLE IF NOT EXISTS posts (
    ...
    -- Índice em data_criacao: ORDER BY data_criacao DESC usa este índice
    KEY idx_posts_data (data_criacao DESC),
...
CREATE TABLE IF NOT EXISTS seguidores (
    ...
    -- Índice adicional: listar_seguidores filtra por id_seguido (não coberto pelo UNIQUE acima)
    KEY idx_seg_seguido (id_seguido),
